...
export const ReportTable = React.forwardRef<HTMLDivElement, ReportTableProps>(
    ({ tickets, period, systemName = 'MTrack System', logoSrc = '/apple-touch-icon.png' }, ref) => {
        ...
        const TableRow = ({ ticket, ...rest }: { ticket: MaintenanceTicket } & React.HTMLAttributes<HTMLTableRowElement>) => (
            <tr className="text-sm break-inside-avoid" {...rest}>
        ...
        const [pages, setPages] = React.useState<Row[][]>([]);

        const renderRow = (row: Row, key: string, dataIndex?: number) => {
            const dataAttrs = dataIndex !== undefined ? { 'data-row-index': dataIndex } : {};

            if (row.type === 'section') {
                return (
                    <tr key={key} className="bg-gray-100/50" {...dataAttrs}>
            ...
            return <TableRow key={key} ticket={row.ticket} {...dataAttrs} />;
        };
        ...
        React.useLayoutEffect(() => {
            const measure = () => {
                ...
                const pageEl = measureRef.current.querySelector<HTMLElement>('[data-measure-page]');
                const headerEl = measureRef.current.querySelector<HTMLElement>('[data-measure-header]');
                const theadEl = measureRef.current.querySelector<HTMLElement>('thead');
                const footerEl = measureRef.current.querySelector<HTMLElement>('[data-measure-footer]');
                const tbodyEl = measureRef.current.querySelector<HTMLElement>('tbody');

                if (!pageEl || !headerEl || !theadEl || !footerEl || !tbodyEl) return;
                ...
                const availableHeight = pageRect.height - paddingTop - paddingBottom - headerHeight - theadHeight - footerHeight;

                if (availableHeight <= 0) return;

                const rowEls = Array.from(tbodyEl.querySelectorAll<HTMLElement>('tr[data-row-index]'));
                const rowHeights = rowEls.map((rowEl) => rowEl.getBoundingClientRect().height);
                ...
                rows.forEach((row, rowIndex) => {
                    ...
                    if (current.length > 0 && height + rowHeight > availableHeight) {
        ...
        }, [rows, period, systemName, logoSrc]);

        return (
            <div ref={ref} className="bg-white text-black w-[210mm] mx-auto box-border">
                <div ref={measureRef} className="absolute left-[-10000px] top-0 opacity-0 pointer-events-none" aria-hidden="true">
                    <div data-measure-page className="w-[210mm] h-[297mm] p-8 box-border">
                        <div data-measure-header className="mb-6">
                        ...
                        <table className="w-full border-collapse border border-black text-sm">
                            ...
                            <tbody>
                                {rows.map((row, rowIndex) => renderRow(row, `measure-${rowIndex}`, rowIndex))}
                            </tbody>
                        </table>

                        <div data-measure-footer className="mt-12 pt-8 grid grid-cols-2 gap-12 text-sm">
                            <div>
                ...
                {(pages.length > 0 ? pages : fallbackPages).map((pageRows, pageIndex) => (
                    <div key={`report-page-${pageIndex}`} className="report-page w-[210mm] min-h-[297mm] p-8 box-border">
                        ...
                        <table className="w-full border-collapse border border-black text-sm">
                            ...
                            <tbody>
                                {pageRows.map((row, rowIndex) => renderRow(row, `${pageIndex}-${rowIndex}`))}
                            </tbody>
                        </table>
